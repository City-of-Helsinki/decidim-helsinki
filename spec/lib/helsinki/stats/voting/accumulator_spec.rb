...
describe Helsinki::Stats::Voting::Accumulator do
  ...
  describe "#accumulate" do
    ...
    it "accumulates the vote time information" do
      ...
      expect(subject[:datetime]["2021-10-04T12:00:00Z"]).to eq(1) # Suomi.fi no postal
      expect(subject[:datetime]["2021-10-04T19:00:00Z"]).to eq(1) # Helsinki
      expect(subject[:datetime]["2021-10-05T08:00:00Z"]).to eq(1) # Managed postal zero
      expect(subject[:datetime]["2021-10-08T22:00:00Z"]).to eq(1) # MPASSid
    end

    it "accumulates the locale information" do
      expect(subject[:locale][""]).to eq(2) # Managed + Managed zero postal
      expect(subject[:locale]["fi"]).to eq(2) # Helsinki + Suomi.fi no postal
    ...
    context "with postal code caching enabled" do
      ...
      it "caches the postal code information" do
        subject
        expect(accumulator.postal_code_votes).to eq(
          "00000" => votes.select { |v| v.user == managed_postal_zero_user || v.user == suomifi_no_postal_user }.map(&:id),
    ...
    context "with citizen" do
      it "accumulates postal codes" do
        expect(subject[:postal]["00000"]).to eq(2) # Managed zero postal + Suomi.fi no postal
      ...
      it "accumulates the age groups and genders" do
        expect(subject[:demographic]["20-29"]).to eq(total: 1, gender: { m: 0, f: 0, neutral: 1 }) # Suomi.fi no postal
        expect(subject[:demographic]["40-49"]).to eq(total: 2, gender: { m: 1, f: 1, neutral: 0 }) # Managed + Managed zero postal
