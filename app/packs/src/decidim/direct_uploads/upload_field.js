...
// This file is overridden to fix a bug with the non-titled multiple file fields
// getting cleared if the user edits their values. This type of field is needed
...
const addSaveButtonEventListener = (modal) => {
  modal.saveButton.addEventListener("click", (event) => {
    ...
    // Fixed a core bug that caused multiple non-titled file fields to get cleared up
    // when the values are edited.
    if (!modal.options.multiple && modal.trashCan.children.length > 0) {
