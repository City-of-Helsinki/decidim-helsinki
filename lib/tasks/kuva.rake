...
namespace :kuva do
  ...
  # 1 EXTRA CONFIG - WHETHER TO SHOW DEBUG MESSAGES IN CONSOLE
  show_debug_console = true

  desc 'Import proposals to Decidim from a .XLSX file.'
  task :import_proposals, [:filename, :useFeature_no, :user_id, :group_id] => [:environment] do |t, args|
    # useFeature_no : integer = under which 'feature' the Proposals will be imported

    quiet = !show_debug_console
  ...
  def import_proposals(args_passed)
    ...
    # Assign cli parameter user to be used later as Author
    p_author = Decidim::User.find(puid.to_i)
  ...
  def handle_timestamp(dt_cell)
    ...
    unixtime = (dt_cell.cell_value.to_f - 25569) * 86400
  ...
  def raw_to_ssp(raw_html)
    ...
    return raw_html.html_safe unless !raw_html
    return nil
  end
  ...
  def gen_body_html(indata_arr)
    ...
    # Add the
    debug_storey << 'Starting with 0 bytes'
